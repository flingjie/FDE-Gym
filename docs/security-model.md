...
inputs. `src/integrations/codex/codex-runtime.ts` re-validates the input
against `roleInputSchema(role)` **before** spawning any model, so a caller that
